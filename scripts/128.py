...
def fetch_and_display_data(mac_address=None):
    ...
    try:
        print("Czekam 10 sekund na wysłanie danych przez ESP...")
        time.sleep(10)  # Dodano opóźnienie 10 sekund
        ...
        if data and isinstance(data, list) and len(data) > 0:
            ...
            if 'temperature' in item:
                ...
                print("---------------------")
                print(f"| Temperatura: {temperature}°C |")  # Wyświetl temperaturę w ramce
                print("---------------------")
                if temperature == 0:
                    ...
                    subprocess.run(["python", "reset.py"]) # Uruchamianie skryptu reset.py
                elif temperature == 85:
                    print("BŁĄD: Nieprawidłowa temperatura.")
                else:
                    print("Temperatura OK.")
            else:
                print("Brak klucza 'temperature' w danych JSON.")
        else:
            print("Brak danych lub dane w nieprawidłowym formacie.")
